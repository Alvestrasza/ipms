...
// Version: v0.2.47 | Created: 2026-08-31 | Last Modified: 2026-09-17
...
constexpr wchar_t k_agent_version[] = L"0.2.47";
...
http_response post_json(const std::wstring& hostname, std::uint16_t port, const std::wstring& path,
  ...
  if (!transport->session) {
    transport->session.reset(WinHttpOpen(L"IPMS-Agent/0.2.47", WINHTTP_ACCESS_TYPE_NO_PROXY,
...
http_response post_binary(const state& identity, const std::string& body, PCCERT_CONTEXT certificate,
  ...
  check_deadline();
  internet_handle session(WinHttpOpen(L"IPMS-Agent/0.2.47", WINHTTP_ACCESS_TYPE_NO_PROXY,
                                      WINHTTP_NO_PROXY_NAME, WINHTTP_NO_PROXY_BYPASS, 0));
...
TransportResult run_gpo_cycle(const std::function<bool()>& cancelled) {
  ...
  try {
    ...
    if (assignment.number("schema") < 3 || assignment.text("operation") == "import_managed_gpo" || assignment.text("operation") == "import_and_link_managed_gpo" || assignment.text("operation") == "activate_managed_gpo") {
