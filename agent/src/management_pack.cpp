...
const std::vector<ManagementPack>& builtin_management_packs() {
  static const std::vector<ManagementPack> packs{
      {"windows-server-core", "0.1.32", AccessMode::read_inventory, {}, {"windows.os", "windows.hardware", "windows.storage", "windows.network", "windows.roles-features"}},
      {"hyper-v-host", "0.1.32", AccessMode::read_inventory, {"windows-server-core"}, {"hyperv.host", "hyperv.virtual-machines", "hyperv.network"}},
  };
  return packs;
}
