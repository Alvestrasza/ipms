...
class AgentLifecycleTests(TestCase):
    def setUp(self) -> None:
        ...
        with zipfile.ZipFile(self.package, "w") as archive:
            archive.writestr("ipms-agent.exe", b"synthetic-agent-0.1.34")
    ...
    def test_administration_inventory_is_tenant_scoped_and_version_aware(self) -> None:
        ...
        self.assertEqual(document["target_version"], "0.1.34")
    ...
    def test_update_job_is_audited_offered_and_completed(self) -> None:
        ...
        self.assertEqual(job.target_version, "0.1.34")
        self.assertEqual(job.artifact_sha256, hashlib.sha256(b"synthetic-agent-0.1.34").hexdigest())
        self.assertTrue(
            AuditEvent.objects.filter(
                tenant=self.tenant,
        ...
        self.assertEqual(binary, b"synthetic-agent-0.1.34")
