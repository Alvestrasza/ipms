...
def tenant_user_payload(membership: TenantMembership, *, actor=None) -> dict:
    ...
    return {
        ...
        "authentication_source": authentication_source(user),
        "manageable": is_tenant_principal(user),
        **tenant_identity_capabilities(membership, actor),
    }
...
class IdentityPasswordSerializer(StrictInputSerializer):
    current_password = serializers.CharField(
        max_length=1024, trim_whitespace=False, write_only=True
    )
    ...
    def validate_new_password(self, value):
        if len(value) < 12:
            ...
            raise PublicApiError("weak_password")
        return value
