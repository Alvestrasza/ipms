 ...
 * Purpose: Expose WSUS reception and comparison in the tenant console.
 */
...
export async function generateMetadata(): Promise<Metadata> {
  return { title: getWsusCopy(await resolveLocale()).title };
}

export default async function WsusPage({
  searchParams,
}: {
  ...
  return (
    <ConsoleShell
      session={session}
      tenant={tenant}
      activeSection="updates-wsus"
    >
      <section className="page-heading" aria-labelledby="wsus-heading">
        <div>
          <p className="eyebrow">{copy.eyebrow}</p>
          <h1 id="wsus-heading">{copy.title}</h1>
      ...
      <WsusConsole
        ...
        initialComparison={overview.comparison}
        initialSourceId={overview.selectedSourceId}
        initialObservedAt={Date.now()}
        available={overview.available}
        tenantId={tenant.id}
        csrfToken={session.csrf_token}
        canManage={hasPermission(tenant, "connectors.manage")}
        locale={locale}
        copy={copy}
      />
    </ConsoleShell>
