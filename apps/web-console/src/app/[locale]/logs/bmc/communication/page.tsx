...
import styles from "@/components/logs-page.module.css";
import { LogsSortHeading } from "@/components/logs-sort-heading";
...
export default async function BmcLogsPage({
  ...
  const logPath = `/${locale}/logs/bmc/communication`;
  ...
  return (
    <ConsoleShell
      session={session}
      tenant={tenant}
      activeSection="logs-bmc-communication"
    >
      <div
      ...
      >
        ...
        <form key={queryString} className="log-filters" method="get">
          <input
            type="hidden"
            name="sort"
            value={first(selected.sort) || "occurred_at"}
          />
          <input
            type="hidden"
            name="direction"
            value={first(selected.direction) || "desc"}
          />
          <fieldset>
          ...
          <div className="log-filters__actions">
            <Link className="outline-button" href={logPath as Route}>
            ...
            <table className="log-table">
              <thead>
                <tr>
                  ...
                  <LogsSortHeading
                    column="severity"
                    label={dictionary.bmcLogs.severity}
                    path={logPath}
                    query={query}
                    copy={logsCopy}
                    defaultSort="occurred_at"
