#!/usr/bin/env bash
# File Name: deploy-wsus-dev.sh
# Version: v0.1.0
# Created: 2026-09-14
# Last Modified: 2026-09-14
# Author: Alice Endelgard
# Organization: Alvestrasza Corporation
# Description: Exact-target IPMS 0.2.42 -> 0.2.43 DEV staging and additive WSUS cutover.
# The installed Agent and its configured package are updated separately, after
# receiver acceptance. Recovery is fenced and manual; this script never restores
...
[[ $EUID == 0 && $# == 6 ]] || { echo 'Usage: deploy-wsus-dev.sh HOST MACHINE_ID PUBLIC_HOST PREVIOUS_SHA RELEASE_SHA --preflight|--stage|--activate' >&2; exit 2; }
expected_host=$1
expected_machine=$2
public_host=$3
previous_ref=$4
...
next_link=/srv/ipms/.current-wsus-next
...
[[ $(readlink -f /srv/ipms/current) == "$previous" && $(<"$previous/VERSION") == 0.2.42 ]]
...
configuration_units=("${units[@]}" ipms-guacd.service nginx.service ipms-certificate-probe.service postgresql.service)
...
broker_acl() {
    # Adding the new column/tables must not alter any existing privilege or owner.
    # Null column ACLs are omitted so the additive inventory column is permitted.
    psql_read "SELECT md5(coalesce(string_agg(value, E'\\n' ORDER BY value),'')) FROM (
        ...
        WHERE n.nspname='public' AND c.relkind IN ('r','p','S','v','m','f') AND c.relname NOT LIKE 'updates\\_%' ESCAPE '\\'
        UNION ALL SELECT 'column:' || c.relname || ':' || a.attname || ':' || a.attacl::text
        FROM pg_attribute a JOIN pg_class c ON c.oid=a.attrelid JOIN pg_namespace n ON n.oid=c.relnamespace
        WHERE n.nspname='public' AND a.attacl IS NOT NULL AND c.relname NOT LIKE 'updates\\_%' ESCAPE '\\'
...
assert_unmigrated() {
    [[ $(psql_read "SELECT count(*) FROM django_migrations WHERE app='updates'
        OR (app='discovery' AND name='0024_software_windows_update_evidence')") == 0 ]]
    [[ $(psql_read "SELECT count(*) FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace
        WHERE n.nspname='public' AND c.relname LIKE 'updates\\_%' ESCAPE '\\'") == 0 ]]
    [[ $(psql_read "SELECT count(*) FROM information_schema.columns WHERE table_schema='public'
        AND table_name='discovery_softwareinventorysnapshot' AND column_name='windows_update_evidence'") == 0 ]]
}
assert_quiescent
assert_administrators
...
receipt=$release/wsus-0243-stage.sha256
stage_state=$release/wsus-0243-stage-state.txt
if [[ $mode == --stage ]]; then
    [[ ! -e $release && ! -L $release ]]
    umask 022
...
protected "$release"
[[ $(git -C "$release" rev-parse HEAD) == "$release_ref" && $(<"$release/VERSION") == 0.2.43 ]]
git -C "$release" diff --exit-code HEAD -- >/dev/null
# Existing ingress, service isolation, broker, dependencies and authentication
# contracts must remain unchanged. Only the reviewed WSUS integration seams may
# differ in the control plane; historical migration files cannot change.
git -C "$release" diff --exit-code "$previous_ref" "$release_ref" -- \
    deploy services/console-broker apps/web-console/pnpm-lock.yaml >/dev/null
"$previous/services/control-plane/.venv/bin/python" - "$previous" "$release" <<'PY'
import ast, json, pathlib, subprocess, sys, tomllib
...
allowed = {
    ...
    'services/control-plane/src/ipms/apps/agent_pki/services.py',
    ...
    'services/control-plane/src/ipms/apps/discovery/models.py',
    'services/control-plane/src/ipms/apps/discovery/migrations/0024_software_windows_update_evidence.py',
    'services/control-plane/src/ipms_control_plane/settings/base.py',
}
assert all(name in allowed or name.startswith('services/control-plane/src/ipms/apps/updates/') for name in changed), 'Unreviewed control-plane source change'
for relative, addition in {
    'services/control-plane/src/ipms_control_plane/settings/base.py': '    "ipms.apps.updates",\n',
    'services/control-plane/src/ipms/apps/core/urls.py': '    path("", include("ipms.apps.updates.urls")),\n',
    'services/control-plane/src/ipms/apps/discovery/models.py': '    windows_update_evidence = models.JSONField(default=dict, blank=True)\n',
}.items():
...
before, after = ((root / relative).read_text() for root in (previous, release))
assert after.replace('"application_version": "0.2.43"', '"application_version": "0.2.42"') == before, 'Unexpected core API view change'
relative = 'services/control-plane/src/ipms/apps/agent_pki/services.py'
trees = [ast.parse((root / relative).read_text()) for root in (previous, release)]
for tree in trees:
    evidence_functions = [node for node in tree.body if isinstance(node, ast.FunctionDef) and node.name == 'confirm_software_inventory']
    assert len(evidence_functions) == 1
    tree.body.remove(evidence_functions[0])
assert ast.dump(trees[0]) == ast.dump(trees[1]), 'Unexpected Agent lifecycle/security service change'
...
expected = {('discovery', '0024_software_windows_update_evidence', False), ('updates', '0001_initial', False), ('updates', '0002_wsus_server_settings', False)}
actual = [(migration.app_label, migration.name, backwards) for migration, backwards in plan]
assert len(actual) == 3 and set(actual) == expected, 'Unexpected pending migration plan'
"
}
assert_pending_migrations
...
django_read shell --no-imports -c "from rest_framework.test import APIRequestFactory; from ipms.apps.core.views import api_information; r=api_information(APIRequestFactory().get('/api/v1/')); assert r.status_code == 200 and r.data['application_version'] == '0.2.43'"
...
if [[ $mode == --stage ]]; then
    ...
    sha256sum "$release/apps/web-console/.next/standalone/server.js" \
        ...
        "$release/services/control-plane/src/ipms/apps/discovery/migrations/0024_software_windows_update_evidence.py" \
        "$release/services/control-plane/src/ipms/apps/updates/migrations/0001_initial.py" \
        "$release/services/control-plane/src/ipms/apps/updates/migrations/0002_wsus_server_settings.py" > "$receipt"
    protected_file "$receipt"
    [[ $(readlink -f /srv/ipms/current) == "$previous" && $(preserved_state) == "$preserved" && $(broker_acl) == "$before_broker_acl" ]]
    echo 'IPMS 0.2.43 candidate staged; exactly three additive migrations pending. Runtime remains 0.2.42. No database or Agent change.'
    exit 0
fi
protected_file "$receipt"
...
backup=$backup_root/0243-$(date -u +%Y%m%dT%H%M%SZ)
[[ ! -e $backup && ! -L $backup ]]
...
plan = executor.migration_plan(executor.loader.graph.leaf_nodes())
actual = [(migration.app_label, migration.name, backwards) for migration, backwards in plan]
assert len(actual) == 3 and set(actual) == {('discovery', '0024_software_windows_update_evidence', False), ('updates', '0001_initial', False), ('updates', '0002_wsus_server_settings', False)}
...
[[ $(psql_read "SELECT count(*) FROM pg_class c JOIN pg_roles r ON r.oid=c.relowner JOIN pg_namespace n ON n.oid=c.relnamespace
    ...
    AND c.relname IN ('updates_updatesource','updates_wsussnapshot','updates_wsuscomputerreport')
...
[[ $(psql_read "SELECT count(*) FROM information_schema.columns WHERE table_schema='public'
    AND table_name='updates_updatesource' AND column_name IN ('wsus_host','wsus_port','wsus_use_ssl','wsus_changed_at')") == 4 ]]
assert_database_owner
assert_administrators
...
curl --connect-timeout 2 --max-time 5 --fail --silent --header "Host: $public_host" --header 'X-Forwarded-Proto: https' \
    http://127.0.0.1:8000/api/v1/ | "$python" -c 'import json,sys; assert json.load(sys.stdin)["application_version"] == "0.2.43"'
for route in en/login de/login api/v1/hyper-v/virtual-machines/00000000-0000-4000-8000-000000000000/management/dialog/ api/v1/auth/account/ api/v1/platform/tenants/ api/v1/service-accounts/ api/v1/update-sources/ api/v1/update-sources/00000000-0000-4000-8000-000000000000/comparison/ api/v1/update-sources/00000000-0000-4000-8000-000000000000/servers/00000000-0000-4000-8000-000000000000/updates/ admin/ admin; do
...
git -C "$release" diff --exit-code HEAD -- >/dev/null
trap - ERR INT TERM
echo "IPMS 0.2.43 DEV receiver activation verified; three additive migrations applied as ipms. Agent package and environments preserved. Installed Agent rollout remains separate. Backup: $backup"
