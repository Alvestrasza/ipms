#!/usr/bin/env bash
# Exact-target, coordinated IPMS 0.2.40 -> 0.2.41 DEV cutover.
...
[[ $EUID == 0 && $# == 6 ]] || { echo 'Usage: deploy-console-input-dev.sh HOST MACHINE_ID PUBLIC_HOST PREVIOUS_SHA RELEASE_SHA --preflight|--stage|--activate' >&2; exit 2; }
expected_host=$1
expected_machine=$2
public_host=$3
previous_ref=$4
...
next_link=/srv/ipms/.current-console-stream-next
...
[[ $(readlink -f /srv/ipms/current) == "$previous" && $(<"$previous/VERSION") == 0.2.40 ]]
...
receipt=$release/console-0241-stage.sha256
if [[ $mode == --stage ]]; then
    [[ ! -e $release && ! -L $release ]]
    umask 022
...
protected "$release"
[[ $(git -C "$release" rev-parse HEAD) == "$release_ref" && $(<"$release/VERSION") == 0.2.41 ]]
git -C "$release" diff --exit-code HEAD -- >/dev/null
# This release only changes console input, liveness and the prompt-free connection workflow.
# No Agent, schema, authorization, credential, dependency or ingress changes.
git -C "$release" diff --exit-code "$previous_ref" "$release_ref" -- agent deploy services/console-broker \
    ...
    services/control-plane/src/ipms/apps/agent_pki/models.py \
    services/control-plane/src/ipms/apps/agent_pki/migrations \
    ...
    services/control-plane/src/ipms/apps/agent_pki/native_views.py \
    apps/web-console/pnpm-lock.yaml >/dev/null
...
# The schema-neutral release must have no pending migration. This performs no DDL.
"$python" "$manage" shell -c "from django.db import connection; from django.db.migrations.executor import MigrationExecutor; e=MigrationExecutor(connection); p=e.migration_plan(e.loader.graph.leaf_nodes()); expected=[]; assert [(m.app_label,m.name,backwards) for m,backwards in p] == expected"
...
"$python" "$manage" shell -c "from rest_framework.test import APIRequestFactory; from ipms.apps.core.views import api_information; r=api_information(APIRequestFactory().get('/api/v1/')); assert r.status_code == 200 and r.data['application_version'] == '0.2.41'"
...
if [[ $mode == --stage ]]; then
    ...
    echo 'IPMS 0.2.41 immutable candidate staged and checked; runtime remains 0.2.40. No migration or Agent change.'
    exit 0
fi
protected_file "$receipt"
...
backup=$backup_root/0241-$(date -u +%Y%m%dT%H%M%SZ)
[[ ! -e $backup && ! -L $backup ]]
...
curl --connect-timeout 2 --max-time 5 --fail --silent --header "Host: $public_host" --header 'X-Forwarded-Proto: https' \
    http://127.0.0.1:8000/api/v1/ | "$python" -c 'import json,sys; assert json.load(sys.stdin)["application_version"] == "0.2.41"'
...
git -C "$release" diff --exit-code HEAD -- >/dev/null
trap - ERR INT TERM
echo "IPMS 0.2.41 DEV activation verified; no schema change. Agent packages, installed Agents, VM state and environments unchanged. Backup: $backup"
