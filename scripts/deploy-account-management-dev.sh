#!/usr/bin/env bash
# Exact-target 0.2.33 -> 0.2.34 DEV cutover. No Agent or firewall changes.
set -Eeuo pipefail
umask 027
...
[[ $(readlink -f /srv/ipms/current) == "$previous" && $(<"$previous/VERSION") == 0.2.33 ]] || exit 2
...
[[ $(git -C "$release" rev-parse HEAD) == "$release_ref" && $(<"$release/VERSION") == 0.2.34 ]]
...
backup=/srv/ipms/backups/identity-0234-$(date -u +%Y%m%dT%H%M%SZ)
...
recover() {
    ...
    if [[ $schema_started == true ]]; then
        ...
        echo 'Do not discard username reservations or restart code that ignores them.' >&2
    else
...
curl --connect-timeout 2 --max-time 5 --fail --silent --show-error --header "Host: $public_host" \
    ...
    | "$python" -c 'import json,sys; assert json.load(sys.stdin)["application_version"] == "0.2.34"'
for route in api/v1/auth/account/ api/v1/platform/tenants/ api/v1/service-accounts/ admin/ admin; do
...
trap - ERR
echo "IPMS 0.2.34 DEV security cutover completed. Account management is available in the portal; existing credentials were not changed. Backup: $backup"
