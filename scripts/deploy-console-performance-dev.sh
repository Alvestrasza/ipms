#!/usr/bin/env bash
# Exact-target, coordinated IPMS 0.2.41 -> 0.2.42 DEV cutover.
...
[[ $EUID == 0 && $# == 6 ]] || { echo 'Usage: deploy-console-performance-dev.sh HOST MACHINE_ID PUBLIC_HOST PREVIOUS_SHA RELEASE_SHA --preflight|--stage|--activate' >&2; exit 2; }
expected_host=$1
expected_machine=$2
public_host=$3
previous_ref=$4
...
next_link=/srv/ipms/.current-console-performance-next
...
[[ $(readlink -f /srv/ipms/current) == "$previous" && $(<"$previous/VERSION") == 0.2.41 ]]
...
receipt=$release/console-0242-stage.sha256
if [[ $mode == --stage ]]; then
    [[ ! -e $release && ! -L $release ]]
    umask 022
...
protected "$release"
[[ $(git -C "$release" rev-parse HEAD) == "$release_ref" && $(<"$release/VERSION") == 0.2.42 ]]
git -C "$release" diff --exit-code HEAD -- >/dev/null
# This portal cutover adds measurements; the source also includes Agent 0.2.29.
# Installed Agent updates remain a separate, explicitly targeted lifecycle step.
...
"$python" "$manage" shell -c "from rest_framework.test import APIRequestFactory; from ipms.apps.core.views import api_information; r=api_information(APIRequestFactory().get('/api/v1/')); assert r.status_code == 200 and r.data['application_version'] == '0.2.42'"
...
if [[ $mode == --stage ]]; then
    ...
    echo 'IPMS 0.2.42 immutable candidate staged and checked; runtime remains 0.2.41. No migration or Agent change.'
    exit 0
fi
protected_file "$receipt"
...
backup=$backup_root/0242-$(date -u +%Y%m%dT%H%M%SZ)
[[ ! -e $backup && ! -L $backup ]]
...
curl --connect-timeout 2 --max-time 5 --fail --silent --header "Host: $public_host" --header 'X-Forwarded-Proto: https' \
    http://127.0.0.1:8000/api/v1/ | "$python" -c 'import json,sys; assert json.load(sys.stdin)["application_version"] == "0.2.42"'
...
git -C "$release" diff --exit-code HEAD -- >/dev/null
trap - ERR INT TERM
echo "IPMS 0.2.42 DEV activation verified; no schema change. Agent packages, installed Agents, VM state and environments unchanged. Backup: $backup"
