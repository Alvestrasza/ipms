...
- First capable Windows Agent: 0.2.10
...
automation representation for the fixed CIM `uint16` state value. On the V2
provider, pause requests `Quiesce` (`9`) and normalizes the resulting stable
state to `paused`; resume requests `Enabled` (`2`). Transitional observations
are never submitted as requested states. Graceful
shutdown uses the
