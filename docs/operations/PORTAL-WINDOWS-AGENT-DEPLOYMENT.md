<!--
File Name: PORTAL-WINDOWS-AGENT-DEPLOYMENT.md
Version: v0.2.76 | Last Modified: 2026-09-20
Author: Alice Endelgard | Organization: Alvestrasza Corporation
Purpose: Deploy the fixed Windows Agent through bounded WinRM bootstrap.
-->
...
IPMS 0.2.76 provides a development-grade portal bootstrap for Windows Agent
0.2.49. The workflow is tenant-scoped, audited, connection-approved, and
...
- **Administration > Tenants > Agent access** reports that the tenant PKI
  recovery custody, exact DNS/SNI Gateway identity, preservation of all other
  tenant endpoints, package SHA-256 and required Agent version are ready.
...
Workgroup membership is supported. The WinRM warning and Agent PKI readiness
are independent: explicitly approved HTTP 5985 can transport the fixed
installer with NTLM and WS-Man message encryption, but enrollment cannot begin
until the tenant Agent access workflow is ready.
...
Agent 0.2.49 also uses well-known security identifiers inside the package. It
...
Until the first inventory succeeds, Agent 0.2.49 retries enrollment every ten
