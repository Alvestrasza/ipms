...
Release `0.2.45`, verified on DEV, adds tenant visibility management, explicit read-only scan
...
- Application 0.2.36 is deployed to the known DEV appliance. The selected host
  ...
  settings and one audit event per lifecycle stage. After graceful shutdown was
  rejected, a separately approved hard stop enabled six successful native
  settings jobs: notes, processor count and startup memory were each changed
  and restored. All reported settings matched the baseline before restart;
  fresh running inspection confirmed the original configuration. This bounded
  acceptance used unchanged software and no wider Agent rollout. Rename, other
  memory edits, checkpoint writes and recovery acceptance remain open.
...
- Application 0.2.38 / Windows Agent 0.2.28 implement state-bound changed-field
  ...
  The application policy is included in activated 0.2.39; the Agent 0.2.28
  canary remains pending. See the [field matrix](docs/architecture/HYPERV-LIVE-SETTINGS-MATRIX.md)
...
- Application 0.2.39 requests an Agent inspection when the management dialog
  ...
  expiry and failed renewal disable editing. Published and activated on DEV at
  `dc026849232086c9100be84eef4e26dc4786d075`; no installed Agent upgrade or VM
  action was included. See the [lease contract](docs/architecture/HYPERV-SETTINGS-DIALOG-LEASE.md)
